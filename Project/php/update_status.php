...
if (!$should_exit) {
    ...
    if (!isset($data['ticket_id']) || !isset($data['status'])) {
        $response['message'] = 'Invalid input. Ticket ID and status are required.';
        $should_exit = true;
    }
}
...
if (!$should_exit) {
    try {
        if ($status === 'closed') {
            ...
            $pdo->commit();
            $response = ['success' => true, 'message' => 'Ticket closed and moved to closed_tickets with associated responses.'];
        } else {
            ...
            if ($is_closed_ticket) {
                ...
                $pdo->commit();
                $response = ['success' => true, 'message' => 'Ticket re-opened and moved to tickets with associated responses.'];
            } else {
                ...
                $response = ['success' => true, 'message' => 'Ticket status updated successfully.'];
            }
        }
    } catch (PDOException $e) {
        ...
        $response['message'] = 'Failed to update ticket status: ' . $e->getMessage();
    }
}
